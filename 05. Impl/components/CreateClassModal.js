    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          classData.id = data.id || `class_${Date.now()}`;
          apiSuccess = true;
        } else {
          console.log('API error status:', response.status);
        }
      } catch (apiError) {
        console.error('Error creating class in API:', apiError);
      }

      // If API failed, generate a local ID
      if (!apiSuccess) {
      ...
      try {
        ...
        const classesJson = await AsyncStorage.getItem('lecturerClasses');
        let classes = [];
        ...
        await AsyncStorage.setItem('lecturerClasses', JSON.stringify(classes));
        ...
        console.error('Error saving class to storage:', storageError);
        Alert.alert('Error', 'Failed to save class data');
      }
    } catch (error) {
