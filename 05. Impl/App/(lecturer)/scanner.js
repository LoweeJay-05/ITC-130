...
// Add a function to find working server
const findWorkingServer = async () => {
  // Try main API URL first
  if (await checkServerConnectivity(API_BASE_URL)) {
    return API_BASE_URL;
  }

  // Try fallback URLs
  for (const url of FALLBACK_URLS) {
    if (await checkServerConnectivity(url)) {
      return url;
    }
  }
...
export default function WebQRScannerScreen() {
  ...
  const [torchMode, setTorchMode] = useState('off'); // 'on' or 'off'
  const [lastScannedStudent, setLastScannedStudent] = useState(null); // New state for scanned student details
  ...
  // Add function to sync pending attendance records
  const syncPendingRecords = async () => {
    if (!workingServer) return;

    try {
      const pendingSyncs = await AsyncStorage.getItem('pendingAttendanceSync');
      if (!pendingSyncs) return;

      const pendingArray = JSON.parse(pendingSyncs);
      ...
      for (const record of pendingArray) {
        try {
          const response = await fetch(`${workingServer}/api/attendance/record`, {
            method: 'POST',
            headers: {
          ...
          if (!response.ok) {
            ...
            if (record.attempts < 3) { // Keep records that haven't been tried 3 times
              updatedPending.push(record);
            }
          }
        } catch (error) {
      ...
      // Update pending syncs in storage
      await AsyncStorage.setItem('pendingAttendanceSync', JSON.stringify(updatedPending));
      
      if (updatedPending.length < pendingArray.length) {
        Alert.alert(
          'Sync Complete',
          `Successfully synced ${pendingArray.length - updatedPending.length} attendance records.`
        );
      }
    } catch (error) {
  ...
  // Add effect to try syncing when we get a working server
  useEffect(() => {
    if (workingServer) {
      syncPendingRecords();
    }
      ...
      try {
        ...
        if (typeof qrData === 'string') {
          ...
          if (pipeMatch) {
            ...
            // --- Display scanned student info temporarily ---
            setLastScannedStudent(studentData);
            // Clear the displayed info after a few seconds (e.g., 5 seconds)
            setTimeout(() => {
      ...
      if (userToken && currentClass) {
        try {
          ...
          try {
            ...
            const directEndpoint = 'http://192.168.156.209:5000/api/attendance/record';
            console.log(`Sending attendance data directly to: ${directEndpoint}`);
            
            // Make the direct API request
            const { success, data, error } = await tryApiEndpoints(attendanceData, userToken);
            ...
            Alert.alert(
              'Network Error',
              'Could not connect to any available servers. The attendance will be stored locally and synced when connection is restored.',
              [{ text: 'OK' }]
            );
            
            // Still add to local scanned students list for later sync
            ...
            try {
              ...
              await AsyncStorage.setItem('pendingAttendanceSync', JSON.stringify(pendingArray));
            } catch (storageError) {
        ...
        <View style={styles.cameraContainer}>
          ...
          {lastScannedStudent && (
            <View style={styles.scannedStudentInfo}>
              <Text style={styles.scannedStudentName}>Scanned: {lastScannedStudent.name}</Text>
              <Text style={styles.scannedStudentDetail}>ID: {lastScannedStudent.studentId}</Text>
              <Text style={styles.scannedStudentDetail}>Email: {lastScannedStudent.email}</Text>
            </View>
          )}
        </View>
      ) : (
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.duplicateModalContent}>
            <View style={styles.duplicateIconContainer}>
              <Ionicons name="information-circle" size={60} color="#FF6B6B" />
            </View>
